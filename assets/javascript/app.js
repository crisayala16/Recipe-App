...
//that contain the specified ingredients
		...
		$.ajax({
			...
			for(var i = 0; i < results.length; i++){
				...
				if(matchingLines === currentRecipe.ingredientLines.length || matchingLines > 0){
					...
					div.append("<a href='recipe.html' target='_blank'><h3 class='text-center' id='recipe-label' order='" + onRecipe + "'>" + recLabel + "</h3></a>");
	...
	$(document).on("click", "#recipe-label", function(){
		...
		database.ref().on("value", function(snapshot){
			...
			var thisLabel = data[clickedRecipe].label;
			console.log(thisLabel);
			$("#recipe-detail").append("<h2 class='text-center'>" + thisLabel + "</h2>");
